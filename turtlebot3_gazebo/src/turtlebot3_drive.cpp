...
void Turtlebot3Drive::update_callback()
{
  ...
  switch (turtlebot3_state_num) {
    ...
    // base case where we want to do a scan and check the direction to go
    case GET_TB3_DIRECTION:

    //check to see if the distance from the from camera is greater than the limit
      if (scan_data_[CENTER] > check_forward_dist) {

        // if there is a wall coming up on the left then change to right turn 
        if (scan_data_[LEFT] < check_side_dist) {
          prev_robot_pose_ = robot_pose_;
          turtlebot3_state_num = TB3_RIGHT_TURN;
        }
        // if there is a wall on the right then change to a left turn
        else if (scan_data_[RIGHT] < check_side_dist) {
          prev_robot_pose_ = robot_pose_;
          turtlebot3_state_num = TB3_LEFT_TURN;
        }
        // if there is nothing on either side then keep driving forward
        else {
